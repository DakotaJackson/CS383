...
public:
    /* Takes a QWidget which is necessary for creating the window and all other subclasses.
     * Takes 4 parameters for directions to initialize the lanes
     */
    explicit JJDisplayOutput(QWidget *parent = 0, int n = 2, int s = 3, int e = 4, int w = 5);

    // Takes an intersection and lane objects to get cars and draw them.
...
private:
    ...
    //Temp
    JPIntersection *m_tempIntersect;
    JPLane *m_tempLane;
